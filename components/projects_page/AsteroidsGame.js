...
const GameBoard = (props) => {
  ...
  let fxExplode = new Sound(explodeSound, 1, 0.2);
  let fxLaser = new Sound(laserSound, 5, 0.1);
  let fxHit = new Sound(hitSound, 5, 0.2);
  let fxThrust = new Sound(thrustSound, 1, 0.1);
  ...
  const update = (ctx, ship, Asteroids, canvas, distBetweenPoints, destroyAsteroid, newGame) => {
    ...
    if(Thrusting && !ship.dead){
      thrust.x += ThrustingSpeed * Math.cos(ShipAngle)/5;
      thrust.y += ThrustingSpeed * Math.sin(ShipAngle)/5;
      fxThrust.play();
    ...
    const onKeyDown = (e)=> {
      ...
      switch(e.keyCode){
        ...
        //rotate ship left (a)
        case 65:
          ShipRot = TurnSpeed / 180 * Math.PI / 90
        break;
        case 37:
          ShipRot = TurnSpeed / 180 * Math.PI / 90
        break;
  ...
  return (
    <div className={styles.asteroidsContainer}>
      <canvas className={styles.gameBoard} ref={canvasRef}></canvas>
      <div className={styles.controls}>
        <div> {'Use "W" or "Up Arrow" to move'}</div>
        <div> {'Use "A/D" or "Left/Right Arrows" to turn'}</div>
